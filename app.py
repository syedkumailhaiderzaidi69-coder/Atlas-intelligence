...
Prepared for: Atlas Intelligence User

----------------------------------------
INVESTMENT PROFILE
----------------------------------------
Investment Goal: {investment_goal}
Portfolio Budget: AED {portfolio_budget:,}
...
TOP RECOMMENDATIONS
----------------------------------------
Primary Area: {portfolio_df.iloc[0]['Area']}
Allocation: {portfolio_df.iloc[0]['Allocation %']}%

----------------------------------------
MARKET SUMMARY
----------------------------------------
Average Investment Score: {df['Investment Score'].mean():.1f}
Average Projected Growth: {df['Projected Growth'].mean():.1f}%
Areas Analyzed: {df['Area'].nunique()}
Properties Analyzed: {len(df)}

----------------------------------------
Generated by Atlas Intelligence
AI-Powered Dubai Real Estate Platform
Contact: syedkumailhaiderzaidi69@gmail.com
========================================
"""
...
st.download_button(
    label="📊 Download Full Market Report (PDF Ready)",
    data=report_bytes.getvalue(),
    file_name=f"Atlas_Report_{datetime.now().strftime('%Y%m%d')}.txt",
    mime="text/plain",
    help="Downloads a complete market analysis report with your selected filters"
)
